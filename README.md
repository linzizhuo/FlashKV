...
**SET 80,302 ops/s、GET 80,417 ops/s，与 Redis 7.0.15 处于同一量级。**
表观差距 7~10% 落在双方测量的标准差范围内，不构成显著差异——详见 [Benchmark 报告](docs/benchmark.md)。
...
> Redis 各轮标准差达 2,000~4,000 ops/s（约 3~5%），与表观差距同量级。在不同虚拟化环境下重复测试，
> 两组数据的相对高低会发生翻转。**结论：FlashKV 与 Redis 7.0.15 基本持平，处于同一量级。**
>
