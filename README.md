...
- **zskiplist** — 跳表，概率平衡 (p=0.25) + span 跨度计算，ZRANK 天然 O(log N)
- **zset** — 有序集合抽象层，dict (member→node, O(1)) + skiplist (score 排序, O(log N))，与 Redis 设计一致
...
| `ZRANGE key start stop` | 3 | ✅ 按 rank 范围取，支持 WITHSCORES |
| `ZREM key member` | 2 | ✅ O(1) 删除 |
| `ZCOUNT key min max` | 3 | ✅ score 区间计数 |
| `ZREMRANGEBYSCORE key min max` | 3 | ✅ score 区间删除 |
...
| 4 | **RDB 持久化** | ~400 行 | 全量快照、序列化格式、COW 思想 |
| 5 | **ZSet 单元测试** | ~100 行 | zset 模块独立测试覆盖 |
