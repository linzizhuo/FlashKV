...
> 补充：这里说的过于绝对——其实存在 2-3 个小成员共用一个字长的情况。但压榨 SDS 的本质手段就是**放弃内存对齐**，所以讨论这个，其实是在讨论"在必须对齐的情况下，压榨变量宽度到底能不能带来优化"。
...
Redis 其实并没有用到 `SDS_TYPE_5`，我们舍弃，所以定义变成了：
...
虽然不做 sdshdr5，但这里讲一下 sdshdr5 的结构，提供一种"极致的优化思路"：
...
// 这是柔性数组优化的一个通用手段，在很多时候，我们操作的主体是buf而非整个字符串，head只是提供一些配置信息而已，这里初学者看到可能会觉得莫名其妙，但只要将buf想成数组指针就可以理解了。
...
> 其实用到静态路由的场景还有**扩容**，但扩容属于 SDS 内部逻辑，在 Redis 源码中夹带了很多私货（指的是一些 Redis 其他模块里的方法、函数、宏），并不干净，所以这里就不扩展了，原理是一样的。